...
public class RedisProCacheManager extends RedisCacheManager {
     ...
     *
     * <p><b>单一 seam</b>:全部 8 个依赖以命名参数显式传入,无任何构造重载。
     * 调用方需传递 {@code null} 表示"该特性未启用"。
     *
     * <p><b>ADR-0057 (Round 43) 收敛</b>:原 9 参构造的 {@code redisCacheRegister} +
     * {@code methodMetadataResolver} 已合并为单一 {@link CacheOperationResolver} seam。
     * 本 manager 持有 1 个 deep 依赖(而非 2 个浅依赖),构造参数 -1。
     *
     * <p><b>为什么不做"便利重载"</b>:
     * <ul>
     ...
     *   <li>Spring 装配路径已稳定,生产仅 1 个 8 参构造,3-参重载从未被生产代码使用
     ...
     *       null-safe 路径生效)</li>
     *   <li>{@code bloomSupport / operationResolver / syncSupport} —— 可为 null(关闭对应特性)</li>
    ...
    /**
     * 实例化 {@link RedisProCache} 的单一 seam — 收敛 7 参构造调用的重复样板
     * (ADR-0016,原 {@code createRedisCache} 与 {@code getMissingCache} 各自
     * 7-arg 重复). <b>ADR-0057</b> 后参数减为 7(原 8):{@code redisCacheRegister} +
     * {@code methodMetadataResolver} 合并为 {@code operationResolver}.
     *
     * <p><b>deletion test</b>:删本方法 → {@code createRedisCache} 与
     * {@code getMissingCache} 恢复各自 7-arg 调用;任一参数新增/重命名时两处
     * 必漏改一边(本 manager 是 Spring 扩展点,Spring 框架可能在 vNext 增删参数).
     *
     * <p>参数契约:与 {@link RedisProCache} 7 参构造完全一致;
