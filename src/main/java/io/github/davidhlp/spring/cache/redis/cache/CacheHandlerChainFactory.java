...
@Slf4j
@Component
class CacheHandlerChainFactory {
    ...
    /**
     * 便捷构造(无 observer bean)— 单元测试用;Spring 装配走 5-arg
     * {@code @Autowired} 构造。
     */
    public CacheHandlerChainFactory(List<CacheHandler> handlers,
                                 ...
                                 ChainEngine engine) {
        this(handlers, properties, meterRegistryProvider, engine, List.of());
    }
    ...
    /**
     * Ordered ChainObserver beans — 标准(MDC/DebugLog/Timer/FiredCounter)与用户
     * observer 均为 Spring Bean,经 {@code List<ChainObserver>} 注入(P1-API-001-C)。
     * 用户只需声明 {@code ChainObserver} Bean 即被自动装配;顺序由 {@code @Order} 决定。
     */
     ...
     * <p>首次调用时：
     * <ol>
     *   <li>注册注入的 ChainObserver 列表到 Engine(MDC / DebugLog / Timer /
     *       FiredCounter + 用户 observer;按 {@code @Order} 排序,按类型去重)</li>
