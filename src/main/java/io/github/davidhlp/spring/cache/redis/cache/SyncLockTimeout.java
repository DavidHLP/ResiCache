...
class SyncLockTimeout {
    ...
    /**
     * 按注解声明的 syncTimeout 解析(经稳定 policy 视图的秒数;{@code -1} = 未覆盖 →
     * 全局配置,{@code 0} = 默认 10s,{@code >0} = 注解值)。
     *
     * @param syncTimeout 注解 syncTimeout 秒数;-1 表示「无方法级覆盖」
     * @return 锁超时秒数(恒为正)
     */
    public long resolveSeconds(long syncTimeout) {
        if (syncTimeout > 0) {
            return syncTimeout;
        }
        if (syncTimeout == 0) {
