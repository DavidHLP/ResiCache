 ...
 *
 * <p>onChainStart 把"调用方原 requestId"
 * 装入 {@link MdcScope} record 返回,onChainEnd 接收该 token 恢复 MDC。
 * observer 状态机完全自承,CacheContext 不承担 stringly-typed 通用 attributes 袋。
 * Engine 不感知 token 内部协议 —— 跨 observer 不混淆,
...
public final class MDCStampChainObserver implements ChainObserver {
    ...
    /**
     * MDC scope token — per-call 状态值对象.
     *
     ...
     * 消费者,未达提升为顶层类型的必要性(YAGNI)。其他 observer 若需类似
