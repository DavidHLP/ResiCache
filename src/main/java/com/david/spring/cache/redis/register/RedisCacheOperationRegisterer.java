...
/**
 * 在Spring Bean初始化后扫描所有方法，
 * 找出自定义的缓存注解并一次性注册它们。
 * 这样就避免了在每次方法调用时都使用AOP来重复注册。
 */
@Slf4j
@Component
...
public class RedisCacheOperationRegisterer implements BeanPostProcessor {
	...
	private void registerCacheableOperation(Object bean, Method method, RedisCacheable redisCacheable) {
		try {
	...
	private void registerCacheEvictOperation(Object bean, Method method, RedisCacheEvict cacheEvict) {
		// 实现与上面类似
	}
