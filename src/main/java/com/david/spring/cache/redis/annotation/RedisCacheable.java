 ...
 * <p>
 * 该注解可以应用于类或方法级别，用于控制方法返回值的缓存行为。
 * 缓存的实现依赖于Spring Cache抽象，同时增加了对Redis特有的过期时间（TTL）支持。
...
public @interface RedisCacheable {
     ...
     * <p>
     * 指定要使用的缓存名称，可以配置多个缓存名称。
     * 当指定多个缓存名称时，方法返回值会被缓存到所有指定的缓存中。
     *
     * @return 缓存名称数组
     * @see #cacheNames()
     */
     ...
     * <p>
     * 此属性与{@link #value()}功能相同，提供更直观的语义。
     * 当同时指定了{@code value}和{@code cacheNames}时，{@code value}属性优先。
     *
     * @return 缓存名称数组
     * @see #value()
     */
     ...
     * <p>
     * 默认情况下，如果未指定key，则使用{@link org.springframework.cache.interceptor.SimpleKeyGenerator}生成键。
     * 可以通过实现{@link org.springframework.cache.interceptor.KeyGenerator}接口来自定义键生成策略。
     *
     * <p>SpEL表达式上下文可用变量：
     * <ul>
     *   <li>{@code #root.method}: 目标方法对象</li>
     *   <li>{@code #root.target}: 目标对象</li>
     *   <li>{@code #root.args}: 方法参数数组</li>
     *   <li>{@code #result}: 方法返回值（仅在{@code unless}表达式中可用）</li>
     *   <li>方法参数名称或索引（如{@code #id}, {@code #p0}等）</li>
     * </ul>
     *
     * @return 缓存键表达式
     ...
     * <p>
     * 当需要更复杂的键生成逻辑时，可以通过实现{@link org.springframework.cache.interceptor.KeyGenerator}
     ...
     * <p>
     * 当需要更复杂的缓存解析逻辑时，可以通过实现{@link org.springframework.cache.interceptor.CacheResolver}
     * 接口并注册为Spring Bean，然后在此处指定Bean名称来使用。
     *
     ...
     * <p>
     * 该表达式在方法调用前求值，如果为{@code false}，则不会缓存方法返回值。
     * SpEL表达式可以使用以下上下文变量：
     * <ul>
     *   <li>{@code #root.method}: 目标方法对象</li>
     *   <li>{@code #root.target}: 目标对象</li>
     ...
     * <p>
     * 与{@link #condition()}不同，此表达式在方法执行后求值，可以访问方法返回值。
     * 如果表达式求值结果为{@code true}，则不会缓存方法返回值。
     * SpEL表达式可以使用以下上下文变量：
     * <ul>
     ...
     * <p>
     * 当设置为{@code true}时，多个线程同时请求同一个缓存键时，
     * 只有一个线程会执行方法体，其他线程会等待结果并返回相同的值。
     * 这可以防止缓存击穿问题，但会降低并发性能。
     *
     ...
     * <p>
     * 设置缓存条目的存活时间（Time To Live），超过指定时间后缓存条目将自动过期。
     * 值为0表示永不过期（不推荐在生产环境中使用）。
     * 负值将被视为使用缓存实现的默认过期时间。
     *
     * <p>注意：
     * <ul>
     *   <li>不是所有的缓存实现都支持TTL功能</li>
     *   <li>某些缓存实现可能有最大TTL限制</li>
     *   <li>实际过期时间可能会有几秒的误差</li>
     * </ul>
     *
     * @return 过期时间（秒），默认60秒
     * @see #sync()
     ...
     * 缓存值的类型，用于类型转换。
     * <p>
     * 指定缓存值的类型，主要用于反序列化时进行类型转换。
     * 当使用JSON序列化时，此属性可以帮助正确反序列化为目标类型。
     * 如果未指定，则使用{@link Object}类型。
     *
     * <p>示例：
     * <pre>
     * {@code
