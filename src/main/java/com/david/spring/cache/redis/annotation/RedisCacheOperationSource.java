...
public class RedisCacheOperationSource extends AnnotationCacheOperationSource {
	...
	private Collection<CacheOperation> parseCacheAnnotations(Object target) {
		...
		if (target instanceof Method) {
			cacheable = AnnotatedElementUtils.findMergedAnnotation((Method) target, RedisCacheable.class);
		} else if (target instanceof Class) {
			cacheable = AnnotatedElementUtils.findMergedAnnotation((Class<?>) target, RedisCacheable.class);
		}
		...
		if (target instanceof Method) {
			cacheEvict = AnnotatedElementUtils.findMergedAnnotation((Method) target, RedisCacheEvict.class);
		} else if (target instanceof Class) {
			cacheEvict = AnnotatedElementUtils.findMergedAnnotation((Class<?>) target, RedisCacheEvict.class);
		}
	...
	private void validateCacheOperation(Object target, CacheOperation operation) {
		...
		if (StringUtils.hasText(operation.getKey()) && StringUtils.hasText(operation.getKeyGenerator())) {
			String errorMsg = "Invalid cache annotation configuration on '" +
					target + "'. Both 'key' and 'keyGenerator' attributes have been set. " +
		...
		if (StringUtils.hasText(operation.getCacheManager()) && StringUtils.hasText(operation.getCacheResolver())) {
			String errorMsg = "Invalid cache annotation configuration on '" +
					target + "'. Both 'cacheManager' and 'cacheResolver' attributes have been set. " +
