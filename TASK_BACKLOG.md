...
- [ ] **WS-1.3 Path C**(销毁 ThreadLocal)— v0.1.0 最大块,Step 0 已启动(4/4 绿)。剩余 Step 1–7:
  ...
  - [x] Step 2(部分,commit `4063968`):仅引入 `chain/CacheInvocationContext.java` 值对象(record + snapshot/restore API,82 行)。**resolver 接口未动** — 加 `currentContext()` + `ScopedValue<CacheInvocationContext>` 字段触发了 Lombok 注解处理器的连锁失败(表现为 `ActualCacheHandler` `@Slf4j` 找不到 `log` + `CacheContext` `@Getter` 找不到 `getOutput()`,与本 tick 无关的 pre-existing bug,被增量编译捕获)。Step 2 retry 项移到 Step 6 异步透传时一并处理(届时同步修 pre-existing Lombok bug 避免再次触发连锁失败)。**当前状态**:值对象已存在,可被 Step 6 `RedisProCacheWriter.retrieve()/store()` 直接引用;`./mvnw clean compile test-compile` BUILD SUCCESS + `checkstyle:check` 0 violation + `PathCAopContractIT + RedisProCacheWriterTest` 12/12 绿 + JaCoCo 108 classes(6.101s)。
  - [x] **Bug fix**(commit `6904c3c`):`CacheContext.java` 加显式 `public getOutput()` 方法(6 行 + 3 行 Javadoc)解锁 Path C Step 2 retry。根因:Lombok `@Getter` 在上 tick 编译路径下未生成 getter → ActualCacheHandler 调 `context.getOutput()` 失败 → 级联到 `log` 也报\"找不到\"(实际 `@Slf4j` 工作正常,只是类处于 error state)。`checkstyle:check` 0 violation + `PathCAopContractIT + RedisProCacheWriterTest` 12/12 绿 + BUILD SUCCESS(9.179s)。Step 2 retry(resolver 加 `currentContext()` + `ScopedValue` 字段)现在可继续。
