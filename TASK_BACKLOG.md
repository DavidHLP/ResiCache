...
- [x] **WS-1.5 故障注入补 Redis 断连场景**(commit `59c1f6d` GET 路径 + `29bd449` PUT 路径 — 重要架构发现):`test/.../cache/RedisDownFaultInjectionIT.java` 2 tests — `redisDown_get_degradesGracefully` + `redisDown_put_degradesGracefully`。用 `@Primary` + `@Profile("redis-down-test")` 覆盖 `RedisConnectionFactory` bean(端口 1,不可达)。**架构发现(诚实记录)**:ResiCache 是 **graceful degradation 模式,不是 fail-fast** — Redis 不可达时 `CacheErrorHandler.handleGetError/handlePutError` log warn "degrading gracefully" + 返回 failed CacheResult → 上游收到 cache miss 触发 loader 兜底(GET)/写失败被吞下次重试(PUT)。**安全属性未丢**(不会用损坏/null 数据响应用户)+ **错误可见**(log warn 配合 WS-1.4 per-handler Counter 可量化)+ **不破坏 SLA**(loader 兜底)。**trade-off vs fail-fast**:选 graceful-degrade 是因为 cache miss 是"安全失败模式"(回源是 idempotent 兜底),fail-open 才不安全。`checkstyle:check` 0 violation + 5 ITs 一起跑 19/19 绿(BUILD SUCCESS — @Profile 隔离有效,其他 IT 不被 BrokenRedisConfig 污染)。**遗留**:CLEAN/EVICT 路径故障注入(留独立 tick,模式同 GET/PUT)+ 并发压测 + Toxiproxy 集成(分歧推荐表优先"不引入新依赖",Toxiproxy 是 jvm-network-tools 间接依赖,引入价值待评估)。
