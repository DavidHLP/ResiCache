...
Redis Cluster 按 slot 分片。旧锁 key = `prefix + key`,可能落到与缓存 key 不同的 slot(不同节点)。虽 Redisson 锁跨 slot 仍有效,但「锁与数据同节点」语义丢失;若日后引入锁内 MULTI / 事务(要求同 slot)将 cross-slot 失败。`buildLockKey(key)`(package-private 供测试):
