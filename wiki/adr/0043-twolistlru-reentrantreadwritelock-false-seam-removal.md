...
- `eviction/TwoListLRU.java`:
  - import 替换:`java.util.concurrent.locks.ReentrantReadWriteLock` → `ReentrantLock`
  - 字段类型替换:`ReentrantReadWriteLock globalLock` → `ReentrantLock globalLock`
  - 6 处 `globalLock.writeLock().lock()` → `globalLock.lock()`(put/get/remove/clear 各 1 处 + try 块)
  - 4 处 `globalLock.writeLock().unlock()` → `globalLock.unlock()`(put/get/remove/clear 各 1 处 + finally 块)
  - 7 处 helper 方法(`promoteNodeUnsafe`/`addToActiveHeadUnsafe`/`demoteOrEvictOldestActiveUnsafe`/`evictOldestInactiveUnsafe`/`insertAfterUnsafe`/`repairChainLinks`/`removeNodeUnsafe`)的 javadoc "需要持写锁" → "需要持有 globalLock 互斥锁"
  - `globalLock` 字段 javadoc 新增 ADR-0043 解释段落 + ReentrantLock 类型
- `wiki/modules/eviction.md`:
  - line 59 "ReentrantReadWriteLock + ConcurrentHashMap" → "ReentrantLock + ConcurrentHashMap"
  - line 64 "并发模型仍是单一 globalLock 写锁保护所有链表操作" → "并发模型仍是单一 globalLock 互斥锁保护所有链表操作(ADR-0043 删 ReentrantReadWriteLock false seam)"
- `wiki/index.md`:追加 ADR-0043 引用
- `wiki/log.md`:追加 round 32 入口

### 验证(JDK 21 via `/home/DavidHLP/jdk21/jdk-21.0.11+10`)

- `mvnw -B checkstyle:check` —— **0 violations / BUILD SUCCESS**
- `mvnw -B test -Dtest='TwoListLRUTest,TwoListLRUConcurrentTest,EvictionStatsTest'` —— **39 tests / 0 failures / 0 errors / 0 skipped**
- `mvnw -B test`(全量)—— **749 tests / 0 failures / 0 errors / 17 skipped**(17 skipped = 需 Docker/Testcontainers 的集成测试,本地环境无 Docker,与本 ADR 无关)
- byte-equivalent:`ReentrantLock.lock()` 与 `ReentrantReadWriteLock.writeLock().lock()` 在独占路径上**100% 行为等价**(同一 monitor 语义、同一可重入语义、同一 fairness)。
