...
- `cache/RedisProCacheWriter.java` —— 删除 line 374-386(`// 以下方法用于向后兼容...` 注释
  + `getTtl(String)` 7 行 + 空行 + `getExpiration(String)` 3 行 + 空行)。文件 395 → 382 行。
  `executeChain` 的 `}` 与 `getChain` 的 javadoc 之间保持单空行分隔。

无 import 变化(`CachedValue` import 在 line 109 `rawValue instanceof CachedValue cv` 仍用,
保留)。

### 验证

- `mvnw checkstyle:check test-compile -B` —— **BUILD SUCCESS**(编译期证明零引用 +
  checkstyle 0 violations)
- `mvnw verify -B`(全量,见 [[log]])
