...
**main**:
- `chain/CacheHandler.java` — 删 `setNext` / `getNext` 声明,javadoc 重写
- `chain/AbstractCacheHandler.java` — 删 `next` 字段 + `getNext()` / `setNext()` + 相关 javadoc
- `chain/CacheHandlerChain.java` — 删 `head` + addHandler O(N) setNext 遍历 + execute/clear 的 head 引用
- `chain/ChainEngine.java` — driveChain index 推进 + executeChainFragment subList + 删 buildFragment
- `protection/breakdown/SyncLockHandler.java` — `getNext()` → `this` + javadoc

**test**:
- 6 文件 mock 删 `getNext` / `setNext` / `next` 样板;`CacheHandlerChainExceptionTest` 4 处自推进改 `continueChain`;`ChainEngineTest.installChain` 删 setNext 循环

### 验证

- `mvnw checkstyle:check` — **0 violations**
- `mvnw test-compile` — **BUILD SUCCESS**
- `mvnw test -Dtest='!*IT'` — unit tests 全过;3 个 `RedisCacheSemanticsIT` 失败(cacheEvict_removesKey / cacheEvict_allEntries_removesAll / cachePut_alwaysExecutesAndUpdates)为 **pre-existing**(bare master `git stash` 本 ADR diff 后跑同一 IT 同样 3 失败,与 ADR-0022 无关;ADR-0019/0020/0021 三次独立验证)
