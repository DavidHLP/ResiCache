...
related:
  ...
  - ADR-0016
  - ADR-0017
  - /tmp/architecture-review-1782889446.html
tags:
  - chain
  - handler
  - observability
...
- **Related**: ADR-0009 (Chain Engine) / ADR-0015 (registerAll) / ADR-0016 (ObserverRegistry) /
  ADR-0017 (Operation.fromAttributes) / `/tmp/architecture-review-1782889446.html` (round 8 报告, 候选 A)
...
**main**:
- `chain/AbstractCacheHandler.java` — 新增 `CounterMetadata` record (3 SLOC) +
  `semanticCounter()` 模板 (4 SLOC body + 30 SLOC Javadoc) +
  `safeIncrementSemantic()` (4 SLOC body + 8 SLOC Javadoc) +
  改写 `attachMeterRegistry` body (8 SLOC); 删除 `safeIncrement(Counter)` (5 SLOC);
  改写 Javadoc 反映新 seam
- `protection/nullvalue/NullValueHandler.java` — 删 1 字段 + 删 1 override
  (5 行) + 删 2 imports + 加 `semanticCounter()` (5 行) + `safeIncrement` → `safeIncrementSemantic`
- `protection/avalanche/TtlHandler.java` — 同上形态
- `protection/bloom/BloomFilterHandler.java` — 同上形态 + 加回 `implements PostProcessHandler`
  (本轮改写时被误删的接口声明)
- `protection/breakdown/SyncLockHandler.java` — 同上形态
- `protection/refresh/EarlyExpirationHandler.java` — 同上形态

**新增 test**:
- `chain/AbstractCacheHandlerSemanticCounterTest.java` (8 contract tests)

### 验证

- `mvnw checkstyle:check` — **0 violations**
- `mvnw test -Dtest='!*IT'` — **BUILD SUCCESS, 775 tests, 0 failures, 0 errors**
  (原 757 + 8 新增 semanticCounter contract + 10 个其他 round 8 累积测试 = 775;
  3 IT 失败为 pre-existing Testcontainers 环境问题, 在 round 8 之前的 master 即存在
  — git stash round 8 diff 后跑 `mvnw test -Dtest='RedisCacheSemanticsIT'` 同样 3 失败)
- `mvnw verify -DskipITs` — **BUILD SUCCESS, All coverage checks have been met**
  (JaCoCo gate 通过, `AbstractCacheHandler.semanticCounter` 与
  `AbstractCacheHandler.semanticCounter()` 与 `safeIncrementSemantic()` 100% 行覆盖)
- 5 个 protection handler 全部 0 改测试通过 (TtlHandlerTest 验证
  `registry.get("resicache.handler.ttl.jittered").counter().count()` 自增契约保持)
