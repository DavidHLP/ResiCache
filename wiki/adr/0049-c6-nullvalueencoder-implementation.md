...
- Round 34 已固化 C8 为 v1.0 毕业议题(`RedisProCacheWriter` SDR 入口扇出),
  Round 36+ 第一候选推进
- Round 35 实施后,`DefaultNullValuePolicy` 与 `NullValueEncoder` 是两类独立
  可替换单元:前者策略面(Strategy 模式),后者实现细节(协作器);两者可独立
  演化而不互相牵动
