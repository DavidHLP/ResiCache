...
C2(AttributeKey 嵌套类删除)在 context check 中**判定为误诊**:
Explore agent 报告称 `CacheContext.java:49-56` 存在 `AttributeKey` 嵌套类,
实际 grep 全仓 `AttributeKey` 仅在 `ObserverRegistry.java:54` javadoc 一处注释中引用,
源码中不存在该嵌套类。**C2 已扼杀,不落地**。

C3 / C4 / C5 见 ADR-0045 / ADR-0046。
