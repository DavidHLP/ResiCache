...
- `chain/ObserverRegistry.java`: **新建** (60 SLOC total / 24 code-only)
- `chain/ChainEngine.java`: 移除 `CopyOnWriteArrayList` import + 字段替换
  (`CopyOnWriteArrayList<ChainObserver>` → `ObserverRegistry<ChainObserver>`)
  + `addObserver` body 委派 + `observers()` body 委派 + 3 处 `for` 循环改
  `forEach` lambda(净 -16 code SLOC)
- `handler/AnnotationChainEngine.java`: 同上模式 + 新增
  `import chain.ObserverRegistry`(净 -3 code SLOC)
- `cache/RedisProCacheManager.java`: 新增 `instantiateRedisProCache(name, cfg)`
  私有方法 + `createRedisCache` / `getMissingCache` body 委派(净 -5 code SLOC)
- `test/chain/ObserverRegistryTest.java`: **新建** (8 contract 测试,184 SLOC)
- `test/cache/RedisProCacheManagerTest.java`: 新增 `InstantiateRedisProCacheContract`
  nested class (3 contract 测试,锁定 D2 行为)
...
- `mvnw test`(全量 unit + integration with Testcontainers)——
  **BUILD SUCCESS,746 tests,0 failures,0 errors**(原 735 + 8 ObserverRegistry
  contract + 3 Manager contract = 746)
- `mvnw verify` —— **All coverage checks have been met** (JaCoCo 70% line /
  40% branch)

### 不变

- 两 engine 的 public `addObserver` / `observers()` 签名零变化
- `RedisProCacheManager.createRedisCache` / `getMissingCache` 签名零变化
- 27+ 现有 `engine.addObserver(...)` 调用方(测试 + 生产)零修改
- `PostProcessHandler` / 5+ handler 子类 / `ChainObserver` / `AnnotationChainObserver`
  行为零变化
...
| 问题 | 状态 | 计划 |
| --- | --- | --- |
| Factory `materialize` builder 填充统一 | round 5 候选 C / 本 ADR 候选 C | 继续 YAGNI |
| `RedisCacheAttributesProjector.from(...)` 3 方法字段映射 | 候选 D | 继续 YAGNI |
| `CacheResult.success()` ≡ `CacheResult.miss()` | 候选 E | 不动 — 语义不同 |
| `CacheHandlerChainFactory` 4 disabled-handler if-block | 候选 F | 继续 YAGNI |
| `ChainEngine.driveChain` 3 处 `result.result() != null ? ... : success()` | 微 DRY 机会 | 不动 — 局部最优 |
| 5 个 `onAttachMetrics` handler 子类 single-counter pattern | 微 DRY 机会 | 不动 — 每 handler unique counter 名 |
