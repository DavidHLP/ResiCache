...
`CacheResultTest` / `CacheErrorHandlerTest` 删死字段断言;`ActualCacheHandlerTest` `isHit`→`getResultBytes`;`BloomFilterHandlerTest` `failure(..)`→`failure()`;`ChainObserverTest` 删 `NoOpTests` + javadoc;`ChainEngineTest` 删 unused import。6 测试类全绿(`mvn test -Dtest=... EXIT=0`,JDK 21.0.10)。
