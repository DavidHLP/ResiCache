...
**main**:
- `protection/refresh/ThreadPoolEarlyExpirationExecutor.java` —— `shutdown()` 收敛为 2 行委派 + 新增 `private shutdownGracefully(...)` helper + javadoc

**test**:零修改。既有 `ThreadPoolEarlyExpirationExecutorTest.ShutdownTests` 3 个测试(含反射取 `executorService` + `cleanupScheduler` 双字段断言 `isShutdown() && isTerminated()` 的 `shutdown_properlyCleansUpResources`)直接保障行为等价。

### 验证

- `mvnw checkstyle:check` —— 0 violations
- `mvnw test -Dtest='ThreadPoolEarlyExpirationExecutorTest'` —— ShutdownTests 3 项全过(行为等价)
