...
related:
  - ADR-0010
  - ADR-0017
  - /tmp/architecture-review-round-10.Txo2.html
tags:
  - annotation
  - reflection
  - polymorphic
...
### 修改(2 files) + 新增(2 files)

**main 修改(2 files, -34 SLOC body / +78 SLOC Javadoc)**:

- `annotation/AnnotationParser.java`:274 → 249 SLOC
  - 6 处 `instanceof + findMergedAnnotation` → 6 个 1-liner `AnnotationTargets.findMerged` 委派
  - 3 处 `instanceof + getName` → 3 个 1-liner `AnnotationTargets.extractTargetName` 委派
  - 3 处中间 `null` 变量消失
  - 类 Javadoc 新增 "Round 10 / ADR-0020 seam 收敛" 段
  - 移除 `Method` import(已无直接使用)
  - 移除 `AnnotatedElementUtils` import(已委派给 helper)

- `annotation/SpringAnnotationAdapter.java`:243 → 234 SLOC
  - 6 对 `hasResiCacheXxx(Method/Class<?>)` 重载 → 6 个多态 `hasResiCacheXxx(Object)` 方法
  - 6 对 `convertSpringCacheXxx(Method/Class<?>, List)` 重载 → 6 个多态 `convertSpringCacheXxx(Object, String, List)` 方法(`String name` 由 `addSpringNativeOperations` 预提取)
  - `addSpringNativeOperations` SELECTIVE / FULL 分支 Method/Class 二分法塌缩为多态
  - `hasResiCacheAnnotation(Object)` Method/Class 二分法塌缩为 `instanceof AnnotatedElement` 守卫 + 多态 helper
  - 类 Javadoc 新增 "Round 10 / ADR-0020 seam 收敛" 段
  - 移除 `Method` import
  - 移除 `AnnotatedElementUtils` import
  - 新增 `AnnotatedElement` import(供 `hasResiCacheAnnotation` 守卫用)
  - 新增 `AnnotationTargets` import(委派)

**main 新增(1 file, 114 SLOC)**:
- `annotation/AnnotationTargets.java`:9 SLOC body + 84 SLOC Javadoc
  - `@UtilityClass` (Lombok) `public final class AnnotationTargets`
  - `public static <A extends Annotation> A findMerged(Object, Class<A>)`
  - `public static String extractTargetName(Object)`
  - 类 Javadoc 详述类型保证、Null 容忍、deletion test、位置选择理由

**test 新增(1 file, 123 SLOC, 9 contract tests)**:
- `annotation/AnnotatedElementPolymorphicSeamTest.java`:
  - `@Nested FindMergedContract` × 6 tests
  - `@Nested ExtractTargetNameContract` × 3 tests
  - 共 9 contract tests 钉住 helper 行为
...
| 问题 | 状态 | 计划 |
| --- | --- | --- |
| 5 ChainObserver 实现 DRY | 候选 E | 继续 YAGNI(ADR-0019 C) |
| 4 disabled-handler if-block | 候选 D | 继续 YAGNI(ADR-0016 F 触发条件未发生) |
| Spring adapter fromAttributes | 候选 F | 继续不动(ADR-0017 C 已封口) |
| Startup 校验器合并 | 候选 G | 继续不动(本轮新增 defer) |
| 1.0 毕业时 int→long 统一 | 候选 H | 1.0 毕业时执行(ADR-0019 D2 显式 defer) |
| 3 个 parse 方法 Builder 模板 | 候选 B | 继续不动(Builder 类型不同,template method 复杂度大于收益) |
| observer 异常处理不一致 | 候选 C | 继续不动(行为差异有意) |
