...
- [2026-07-09] improve | Round 50 | `/improve-codebase-architecture` 扫 **`annotation/`** 域剩余架构摩擦(前 49 轮几乎未触及注解 → Builder 填充路径):HTML 评审 `/tmp/architecture-review-1783571986.html` 列 3 候选(Strong 1 / Worth exploring 1 / Speculative 1),唯一落地候选 A(Strong)**BuilderPopulator deep seam 抽出** —— `AnnotationParser` 3 个 parse 方法 × 6 个文本字段 × `if (StringUtils.hasText) setX` 样板(18 处)+ `SpringAnnotationAdapter` 3 个 build 方法 × 17 处私有 `applyText` 调用,6 个近镜像 builder 方法的"setName + setCacheNames + 6 文本字段 + 1-2 special 字段 + build"形状收口到新 `@UtilityClass` seam `BuilderPopulator`(`@UtilityClass` Lombok 生成 final + private 构造);暴露两个 seam helper:`applyText(B, String, BiConsumer<B, String>)` 单字段 null-safe 写入(用 BiConsumer 而非原 `Consumer<String>`,允许 setter 引用捕获 builder 实例,适配 Lombok `setX` 链式 + Spring 标准 `Builder::setKey` 两种签名)+ `populate(B, A, List<TextField<A,B>>, List<BiConsumer<B,A>>)` 编排 helper(迭代 textFields 应用 hasText 守卫 + 迭代 specialFields 无条件应用 + 链式返回 builder);`TextField<A,B>` record 承载 (value-getter, builder-setter) 二元组 + 静态工厂 `textField(value, setter)` 让 caller 写 `textField(Anno::key, B::setKey)` 而不是 `new TextField<>(...)`;`AnnotationParser` 6 个 if-守卫 × 3 parse 方法 + `SpringAnnotationAdapter` 17 applyText 调用 + 1 私有 `applyText` helper 全部删除;3 parse 方法退化为"setName + setCacheNames + 1 populate(...) + build()"4 步,3 build 方法退化为"setName + setCacheNames + 1 populate(...) + build()"4 步;**name + cacheNames 不进 populate** —— 这 2 字段 setter 签名 builder-specific,且"value vs cacheNames 合并"逻辑各 caller 不同,塞进 populate 会增加 4 个参数(annotation-specific getter + 2 个 setter)大于收益;**3 备选路径全部驳回**:A(只抽 applyText helper 不做 populate 编排)→ 6 个方法仍然 6 份独立 shape 实现,seam 浅度名义化;B(把 name + cacheNames 也塞进 populate)→ 参数膨胀 + 失去 caller-side"value vs cacheNames 合并"灵活性,违反 ADR-0029 hypothetical seam 拒绝;C(走 reflection-by-name 而非方法引用)→ 失去编译期类型校验,违反 ADR-0009 typed seam 原则;**净 +180 SLOC 全为 Javadoc + seam 骨架**,代码行(不含 Javadoc)**-30 行**(`AnnotationParser.parseRedisCacheable/Evict/Put` 各减约 6 行 + `SpringAnnotationAdapter.buildRedisCacheableOperation/PutOperation/EvictOperation` 各减约 5 行 + 私有 `applyText` 删除);**公开 API / 注解解析语义 / Spring 装配 / 操作 byte-equivalence 全部 byte-equivalent**(parseResiCacheAnnotations / addSpringNativeOperations / build 操作产物逐字段对齐);`populate` 对 null `textFields` / `specialFields` 防御性视为空列表(原 applyText 不会出现 null list,纯防御无回归风险);**815 tests / 0 fail / 0 err / 17 skipped** ✓(较 Round 49 的 802 增 13 = 4 applyText 行为 + 7 populate 编排 + 2 TextField record 契约);checkstyle 0 violations;JaCoCo 70%/40% gate met;1 新文件 `BuilderPopulator` + 1 新文件 `BuilderPopulatorTest` + `AnnotationParser.java` 重构(18 if-守卫→ 1 populate spec)×3 + `SpringAnnotationAdapter.java` 重构(17 applyText→ 1 populate spec + 私有 applyText 删除)×3

---

## 2026-07-09 (round 49)

- [2026-07-09] improve | ADR-0062 | Round 49 `/improve-codebase-architecture` 兑现 Round 48 遗嘱「loader 路径编排可进一步下沉」—— 扫 **`cache/RedisProCache`** 域剩余架构摩擦:HTML 评审 `/tmp/architecture-review-1783569191.html` 列 3 候选(Strong 1 / Worth exploring 1 / Speculative 1),用户要求"一次做完不留尾巴" → 唯一落地候选 1(Strong)**LoaderOrchestrator deep seam 抽出** —— `RedisProCache.get(key, loader)` 的 3 步编排 + 嵌套的 3 个 package-private seam(`isBloomShortCircuited` / `loadValue` / `performLockedLoad`)+ 2 个 private helper(`executeSyncLoad` / `resolveSyncTimeout`)整体抽到新 `@Component-free` 类 `LoaderOrchestrator`,3 个协作 bean(`BloomGate` / `SyncSupport` / `SyncLockTimeout`)从 `RedisProCache` 字段下沉到 `LoaderOrchestrator` 构造依赖,`RedisProCache` 持有 1 个 `LoaderOrchestrator` 字段 + `operationResolver` 唯一 collab;orchestrator 公开 `orchestrate(...)` 返回 sealed `LoadOutcome<T>`(`BloomShortCircuited` / `Loaded` / `LoadFailed` 三态 record,Java 21 pattern matching 强制 caller 编译期覆盖全部分支);orchestrator 不继承 `RedisCache`,通过 4 个 callback + 1 个 `cacheName` 参数与 `RedisProCache` 解耦:`Function<Object, String> redisKeyFn`(键派生)+ `Function<Object, ValueWrapper> doubleCheckFn`(锁内双检,走 `super.get` 绕过 override 不打 metrics)+ `BiConsumer<Object, Object> putAfterLoad`(锁内写回,走 override 保留 putTimer + putCounter)+ `DefaultLoadFn<T> defaultLoadFn`(`@FunctionalInterface` 显式声明,委派 `super.get(key, loader)` 走 Spring local-lock)+ `redisKeyFn` 用于 `BloomGate.definiteMiss` 与 `SyncSupport.executeSync` lockKey;`RedisProCache.get(key, loader)` 主体退化为 timed wrap + orchestrator.orchestrate + switch 翻译 3 态(BloomShortCircuited 自增 miss + return null / Loaded 直接 return / LoadFailed 自增 miss + throw translateFailure(cause, key));**3 备选路径全部驳回**:A(只做 seam 命名不抽类,3 seam 仍在 `RedisProCache`)→ 浅度名义化 deletion test 反向;B(走抽象 `LoaderSink` interface 不耦合 callback)→ 接口≈实现 shallow 模块;C(拆 `SingleFlightCoordinator` + `LockBackendHealth` 两个 seam)→ 拆分未达 ADR-0029 双适配器门槛,过度拆分违反 YAGNI;**净 +95 SLOC 全为 Javadoc + seam 骨架**,代码行(不含 Javadoc)**-25 行**;`RedisProCache` 字段从 12 缩到 9(bloomGate/syncSupport/syncLockTimeout 3 字段删除);3 旧 test class(PerformLockedLoadTests / C3IsBloomShortCircuitedTests / C3LoadValueTests)**整体迁出**至新 `LoaderOrchestratorTest`(11 单测 = 4 bloom + 1 sync 路由 + 4 locked-load + 2 default load,零 RedisProCache fixture 直接覆盖 3 决策分支);`RedisProCacheTest` 3 个新 `GetWithLoaderIntegrationTests` 验证 orchestrator 委派 + miss counter 时机(bloom 短路 1 次 / 失败路径 1 次 / 成功路径 0 次,字节等价 Round 47/ADR-0057);**公开 API / 序列化字节 / Redis 写入序列 / Spring 装配 / 注解默认值 / Spring Cache local-lock 路径全部 byte-equivalent**;**802 tests / 0 fail / 0 err / 17 skipped** ✓(较 Round 48 的 797 增 5 = 4 locked-load 路径迁移 + 3 default load 新增 - 1 旧 performLockedLoad 重复 setup);checkstyle clean;JaCoCo 70%/40% gate met;1 新文件 `LoaderOrchestrator` + 1 新文件 `LoaderOrchestratorTest` + `RedisProCache.java` 重构 + `RedisProCacheTest.java` 重构(3 旧 test class 整体迁出至 LoaderOrchestratorTest)
