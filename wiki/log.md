...
- 状态: STOPPED-PUBLISH-BLOCKED —— 用户已批准发布,outward gate 转执行模式;release.yml 部署失败,受阻于 Central 版本占用 + pom 签名配置,等用户版本号决策
- current-target: v0.0.3(已被 Central 旧代码占用,不可重发)→ 候选 **0.0.6 / 0.1.0**(Central 均 404 可用)
- 最后更新: 本轮 docs commit(轮36,见下)
- BLOCKED 计数器: 0
...
- 发布解锁(待用户决策,非执行型 gate):
  1. **选新版本号**:0.0.6(续 0.0.x 线,最小语义跳跃)/ 0.1.0(标记 Boot 4 里程碑)
  2. **pom 签名修复**(版本无关,可先做):sign-artifacts `<phase>deploy</phase>` → `<phase>verify</phase>`(签名在 verify 完成,deploy 时 bundle 已含签名)→ 改后 `./mvnw clean verify -B` 全门绿
  3. **CHANGELOG `v0.0.3` 段重命名**为选定版本(line 79 `## [Unreleased] — planned for v0.0.3` + 段内引用)
  4. **删失败 tag + retag**:`git push origin :refs/tags/v0.0.3` + 本地 `git tag -d v0.0.3`,新版本 retag + push 触发 release.yml
  5. **发布成功后 GitHub Release**:softprops `if:success()` 自动建,body 用 CHANGELOG 内容
  6. **本地 tag 收尾**:本地仍残留 `v1.0` + `v0.0.3`(远程 v1.0 已删,v0.0.3 待替换),发布方案定后统一 `git tag -d`
